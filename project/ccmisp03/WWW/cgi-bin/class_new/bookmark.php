...
#          echo "insDoc(aux2, gLnk(\"R\", \"支援本班課程\", \"Support_Courses.cgi?session_id=" . $session_id . "\"))\n";
  ...
  aux2 = insFld(foldersTree, gFld("資料查詢", ""))
    <?PHP
      ...
      echo "insDoc(aux2, gLnk(\"R\", \"進階開課資料查詢\", \"" . $QUERY_URL . "Query_by_time1.cgi?session_id=" . $session_id . "&get_my_table=1\"))\n";
      echo "insDoc(aux2, gLnk(\"R\", \"支援本班課程\", \"Support_Courses.cgi?session_id=" . $session_id . "\"))\n";    
//      echo "insDoc(aux2, gLnk(\"R\", \"檢視所有學程開課資料\", \"" . "Show_All_GRO.cgi\"))\n";
      echo "insDoc(aux2, gLnk(\"R\", \"跨領域學程\", \"Show_All_GRO.cgi?session_id=" . $session_id . "\"))\n";       
      echo "insDoc(aux2, gLnk(\"R\", \"所有異動科目\", \"" . $HOME_URL . "Update_Course.html\"))\n";
...
//    echo "login time = " . $session_data{login_time} . "<BR>\n";
  ...
  <?PHP 
    ...
    if(  1  ) {                                				 //  違章建築 2008/06/04
      echo("<CENTER><TABLE border=0><TR><TD><HR></TD></TR><TR><TD bgcolor=YELLOW>");
      echo("<FONT color=RED size=-1>提醒您, 選課密碼請定期更新(最好三個月一次), 以測安全!<FONT>");
      echo("</TD></TR><TR><TD><HR></TD></TR></TABLE></CENTER>");
    }

    if( $use_default_password == 1  ) {					//  如果使用預設密碼
      echo("<CENTER><TABLE border=0><TR><TD><HR></TD></TR><TR><TD bgcolor=YELLOW>");
      echo("<FONT color=RED size=-1>請先更新您的密碼<FONT>");
      echo("</TD></TR><TR><TD><HR></TD></TR></TABLE></CENTER>");
    }
  
    if( Need_Special_Message($student{'id'}) ) {
      echo("<TABLE border=0><TR><TD><HR></TD></TR><TR><TD bgcolor=YELLOW>");
      echo("<FONT color=RED size=-1><A href=\"special_message.html\" target=basefrm>您有一封來自系統的訊息!!</A><FONT>");
