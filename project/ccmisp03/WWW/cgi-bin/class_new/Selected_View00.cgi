...
if( $Input{session_id} eq "" ) {			###  如果是從別的系統連過來的
  ...
  ($id, $Input{password}, $login_time, $ip) = Read_Session($Input{session_id});
...
@Credit=CREDIT_TABLE();
  ...
  $DATA = $DATA."  <th bgcolor=yellow><font size=2>大綱</font></th>\n";  
  $DATA = $DATA."</tr>\n";

  ####  列印相關課程資料  ####
  for($i=0; $i < $MyCount; $i++){
      my(%theCourse)=Read_Course($MyCourse[$i]{dept},$MyCourse[$i]{id},$MyCourse[$i]{group},$yearterm);
      ...
      $DATA=$DATA."<A href=\"".$ECOURSE_QUERY_COURSE_URL."&courseno=".$theCourse{id}
        ."_".$theCourse{group}."&year=".$YEAR."&term=".$TERM."\" target=NEW>"
        ."連結</A>";
      $DATA=$DATA."</font></th>\n";
...
sub Create_Course_Time_Table()
{
  ...
  foreach $course (@MyCourse) {
    %the_Course = Read_Course($$course{dept}, $$course{id}, $$course{group}, $yearterm);
...
sub Warn_of_Course_Conflict()
{
  ...
  foreach $stu_course (@Course_of_Student) {
    %The_Course = Read_Course($$stu_course{dept}, $$stu_course{id}, $$stu_course{group}, $yearterm, $Student{id});
