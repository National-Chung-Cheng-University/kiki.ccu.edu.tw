...
sub Read_Immune_Record()
{
  ...
  my $immune_path = $DATA_PATH . "Course_Upper_Limit_Immune/";
...
sub Add_Immune_Record()
{
  ...
  my($immune_file) = $DATA_PATH . "Course_Upper_Limit_Immune/" . $c_id . "_" . $c_group;
  my(@line);
  open(IMMUNE, $immune_file);
  @line = <IMMUNE>;
  close(IMMUNE);
  ...
  foreach $list (@list) {
    ...
    qq(
      <TR>
        ...
        <TD>$$list{stu_id} $$S{$$list{stu_id}}{name}</TD>
      </TR>
    );
  }
