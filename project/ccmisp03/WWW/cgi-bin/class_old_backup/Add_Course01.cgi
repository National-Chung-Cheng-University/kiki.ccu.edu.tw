...
%Input=User_Input();
($Input{id}, $Input{password}, $login_time, $ip, $add_course_count) = Read_Session($Input{session_id});
...
if($pre_course_count > 0) {        ###### 有先修科目的話, 顯示警訊html檔
  $PRE_COURSE_WARNING = qq(
    <SCRIPT language="javascript">
      messageWindow = open('Prerequisite_course_warning.html', 'messageWindow', 'resizable=yes, width=250, height=250');
    </SCRIPT>
  );
}
if( $Input{dept} eq "7306" ) {     ######  修教育學程的課, 必須是教育學程的學生html warning
  $EDU_COURSE_WARNING = qq(
    <SCRIPT language="javascript">
      messageWindow = open('Education_course_warning.html','messageWindow', 'resizable=yes, width=250, height=250');
    </SCRIPT>
  );
}
...
if( $Input{dept} eq "V000" ) {     ######  軍訓課程說明 (added 2006/11/14)
  $MIL_COURSE_WARNING = qq(
    <SCRIPT language="javascript">
      messageWindow = open('military_info.html','messageWindow', 'resizable=yes, width=350, height=350');
    </SCRIPT>
  );
}
...
if( $Input{dept} eq "F000" ) {     ######  體育課程說明 (added 2006/11/24)  
  $MIL_COURSE_WARNING = qq(
    <SCRIPT language="javascript">
      messageWindow = open('physical_info.html','messageWindow', 'resizable=yes, width=450, height=450');
    </SCRIPT>
  );
}
...
if( $Input{dept} eq "Z121" ) {     ######  語言中心課程說明 (added 2006/12/26)
  $MIL_COURSE_WARNING = qq(
    <SCRIPT language="javascript">
      messageWindow = open('lang_msg.html','messageWindow', 'resizable=yes, width=800, height=600');
    </SCRIPT>
  );
  $COURSE_WARNING2 = qq(
    <TABLE border=0 width=70%>
      <TR><TD bgcolor=YELLOW>
        <CENTER>選修語言中心課程須知 </CENTER>
        <FONT size=-1>
        <OL>
      <LI> 語言中心課程僅開放第一階段選課系統選課，第二階段不開放選課系統選課，由語言中心以加簽方式辦理。
      <LI> 選修語言中心課程者，開學第1週務必出席了解課程要求，若第1週未出席，請自行退選，否則該課程必需在棄選期間辦理棄選或期末成績以0分計算。
      <LI> 未參加任何校外英檢者，請自行評估自己程度以選擇適當程度修課，若不確定，則可先至語言中心參加檢核測驗或預約諮詢，以取得上課核可或協助評估程度。
      <LI> 語言中心開設之課程視同英文能力畢業門檻第３階段，不再另行開設密集式之加強課程。                  
      <LI> 語言中心將新增「進階日語聽讀訓練」，有意選修者，請見語言中心網站說明 (
           <A href="http://cculc.ccu.edu.tw/modules/news/article.php?storyid=315" target=NEW>
            http://cculc.ccu.edu.tw/modules/news/article.php?storyid=315</A>)
        </OL>
      </TD></TR>
    </TABLE>
    <P>
