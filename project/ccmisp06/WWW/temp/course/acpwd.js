function star(){	
	...
	if(c<=4){
		...
		document.form1.usercd.value=temp;
		...
		if(ls_flag==1){			
			document.form1.usercd.value=document.form1.temp_count.value.toUpperCase();
			for(i=1;i<=c;i++){
				temp=document.form1.temp_count.value.charAt(i-1);
				if (i>4){
					ls_data = ls_data + '*';
			...
			// 帳號長度不一樣時,才觸發此段程式			
			if(document.form1.temp_count.value.length != document.form1.usercd.value.length){
				temp=document.form1.temp_count.value.charAt(c-1);
				temp=temp.toUpperCase();
				document.form1.usercd.value += temp;
				temp=document.form1.usercd.value.substr(0,4);
				temp=temp.toUpperCase();
				for(i=4;i<c;i++)
				{temp +=  '*' ;}
				document.form1.temp_count.value=temp;
...
function Submitform()
{
	var user =document.form1.usercd.value;
...
function sf()
{
	...
	if (user!=null)
		{
			document.form1.usercd.value=user;
