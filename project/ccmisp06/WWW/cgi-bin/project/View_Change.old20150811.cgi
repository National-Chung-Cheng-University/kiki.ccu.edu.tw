...
#####  2015/05/27 修正語言中心可開設通識外語課資料帶出問題 Nidalap :D~
#####  2015/08/11 文學院課程委由各系開設（切換身份）相關修改  Nidalap :D~
...
Write_HTML_File_Header();
 ...
 if( $Input{'open_dept'} eq $DEPT_LAN ) {										#####  若是語言中心，額外抓取通識外語課程
   ...
   @Old_Course = Find_All_Course($Input{'dept_cd'},"","change");
 }
...
######################################   檢查已取消的科目 ########################################
 ...
 {
   #####  若是語言中心登入且課程為通識外語，則 $dept2 為通識。正常情形 $dept2 = $Input{'open_dept'}
   if( ($Input{'open_dept'} eq $DEPT_LAN) and ($Course[$j]{id} =~ /^7102.../) ) {
     $dept2 = $DEPT_CGE;
   }else{
     $dept2 = $Input{'open_dept'};
   }
...
#    Log2($dept,"C");
 ...
 for($i=0;$i < $count;$i++)
 {
   ...
   #####  若是語言中心登入且課程為通識外語，則 $dept2 為通識 $Input{'dept_cd'}。正常情形 $dept2 = $Input{'open_dept'}
   if( ($Input{'open_dept'} eq $DEPT_LAN) and ($Course[$j]{id} =~ /^7102.../) ) {
     $dept2 = $DEPT_CGE;
   }else{
     $dept2 = $Input{'open_dept'};
