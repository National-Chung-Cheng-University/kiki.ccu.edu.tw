...
#####   2016/08/18 修正學系服務學習課程不應顯示篩選原則、限修人數等欄位之 BUG  Nidalap :D~
#############################################################################################
...
############################################################################  
  ...
  if( $is_dept_serv == 1 ) {		###  系所服務學習課程只能選一小時
...
{
  print qq(
    <tr><th bgcolor=yellow>人數篩選</th>
    <th colspan=3 align='LEFT'>篩選原則：<select name=principle>\n);

  my($p,@p_string, $p_default);
  $p_string[0]="不需篩選（碩博士班或特殊科目）";
  $p_string[1]="一次篩選（學士班）";
  $p_string[2]="二次篩選（第一學期學士班大一必修）";

  ###  設定篩選原則的預設值 : 第一學期的大一，預設為二次篩選
  $p_default = 2  if( ($TERM == 1) and ($Course{grade} eq "1") and ($Input{dept_cd} !~ /6$/) );
  ###  設定篩選原則的預設值 : 預設為本科目原先的篩選原則
  $p_default = $Course{principle}  if( $Course{principle} ne "" );
  $p_default = 0  if( $is_dept_serv );
  ...
  if( $SUPERUSER == 1 ) {                    #####  管理者, 可以修改限修人數   
    ...
    print("$Course{number_limit}(由教室容量決定)");
  }
  ...
  ### 保留學生名額 ###
  print "保留新生名額：";
  print "<select name=reserved_number_2>";
  my($i,$j);
