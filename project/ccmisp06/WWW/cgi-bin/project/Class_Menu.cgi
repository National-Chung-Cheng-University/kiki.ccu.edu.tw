...
if( $DEPT_NEED_TO_CHANGE_PASSWORD == 1 ) {
  ...
  $html_batch_open	= Print_Button("Batch_Open_Course_1.cgi", "複製上學年同學期開課資料");
  $html_switch_dept	= Create_Switch_Dept_HTML();			###  切換系所身份

  if( $Input{open_dept} eq $DEPT_PHYSICS ) {
    $html_early_warn	= Print_Button("Early_Warning_21_List.php", "物理系輔導學生選課機制");
  }
  if( $Input{open_dept} eq $DEPT_EDU ) {
    $html_edu_stu_cou	= Print_Button("EDU_Stu_Courses.php", "檢視師培生選師培課程");
  }
  if( $Input{open_dept} eq $DEPT_CGE ) {
...
############################################################################################
sub Create_Switch_Dept_HTML
{
  my $html_switch_dept = "";
  if( ($can_switch = Dept_Can_Switch($Input{open_dept})) > 0 ) {
    if( $Input{dept_cd} ne $Input{open_dept} ) {
	  ...
	  $html_switch_dept	= Print_Button("Switch_Open_Dept.php?switch=1", "切換身份開設" . $new_dept . "課程");
	}
  }
  return $html_switch_dept;
}
############################################################################################
