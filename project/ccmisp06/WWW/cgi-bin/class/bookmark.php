<?PHP
  ...
  /////    2013/10/01 加入行動版相關判斷與畫面最佳化 by Nidalap :D~
  /////    2014/06/05 行動化小改版：第一次進來(POST)時轉址給自己透過 GET 傳值，以避免回來時還要reload。 by Nidalap :D~
  /////    2015/01/20 畢業資格審查表不再只給大三以上看，開放給所有大學生和研究生了(研究生則是成績單pdf)  by Nidalap :D~
...
//	Check_System_Type_And_Redirect($id);		/////  檢查是否該轉址到專班/一般系統
  ...
  if( isset($_GET["session_id"]) ) {
    ...
    if( $IS_MOBILE ) {
	  Student_Log("LoginM ", $id);	///  若是由密碼輸入頁面連結過來 -> 紀錄 Login(行動版額外紀錄)  added 20140530
	}else if( $IS_ENGLISH ) {
	  Student_Log("LoginE ", $id);	///  若是由密碼輸入頁面連結過來 -> 紀錄 Login(英文版額外紀錄)  added 20140612	
	}else{
...
//  $ban_res_time = 1223;
  ...
  if( isset($_POST["password"]) )	header("Location:bookmark.php?session_id=$session_id&m=$IS_MOBILE&e=$IS_ENGLISH");
?>
...
<HTML>
  ...
  <?PHP 
    if( $IS_MOBILE >= 1 ) {						///  行動化介面
	  if( !isset($_GET["session_id"]) ) {		///  第一次進來(POST)：轉址給自己透過 GET 傳值，以避免回來時還要reload
	    header("Location:bookmark?session_id=$session_id&m=1");
	  }else{
	    echo $MOBILE_META_TAG;
	  }
	}
  ?>
...
//
  ...
  aux2 = insFld(foldersTree, gFld("<?PHP echo $txt['about']; ?>", ""))
    <?PHP 
     ...
     if( $system_settings{"restrict_stdno2"} ) {	//  如果有開啟「限制不可選課學號」
        $temp = $system_settings{"restrict_stdno2"};
        //if( !preg_match("/^.$temp.+/", $student{"id"}) and !($css{$student{"id"}} == 1) ) {
		//if( !preg_match("/^.$temp.+/", $student{"id"}) and !(array_key_exists($student{"id"}, $css) ) ) {
		if( ( $temp == substr($student{"id"},1,2) ) and ($student{"enrollnum"}==0  ) ) {
          $add_flag = 0;			 	  //  符合此學號條件，且註冊次數為0者，不可選課。
		  //echo "ccc";
		}
      }
...
//      echo "add_flag = $add_flag!\n";
      ...
      if ( $add_flag ) {
        if( $ban_flag and !($SUPERUSER) ) {							//  被停權
	      ...
	      if( !$IS_MOBILE ) {
		    ...
		    $m_txt['ban'] = "<A href='Show_Ban_Message.php?ban_res_time=$ban_res_time&m=1' rel='external'>停權公告</A>";
		  }
        }else{											//  沒有被停權
          if( !$IS_MOBILE ) {
		    ...
		    $m_txt['add'] = "<A href='Add_Course00.cgi?session_id=$session_id&m=1' rel='external'>" 
						  . "<IMG src='../../Graph/mobile/add_course.png' width='$ICON_WIDTH'><BR>". $add_word . "</A>";
			if( $add_flag == 1 )
			  $m_txt['del'] = "<A href='Del_Course00.cgi?session_id=$session_id&m=1' rel='external'>" 
							. "<IMG src='../../Graph/mobile/del_course.png' width='$ICON_WIDTH'><BR>退選</A>";
		  }
        }
      }
      ...
      if( ($system_settings{'sysstate'} >= 1 ) or ($SUPERUSER == 1)) {
		if( !$IS_MOBILE ) {
		  ...
		  $m_txt['view'] = "<A href='Selected_View00.cgi?session_id=$session_id&m=1' rel='external'>" 
		    . "<IMG src='../../Graph/mobile/selected_view.png' width='$ICON_WIDTH'><BR>已修科目</A>";
		}
      }
      ...
      if( !$IS_MOBILE ) {
	    ...
	    $m_txt['view_warning'] = "<A href='View_Warning.cgi?session_id=$session_id&m=1' rel='external'>檢視篩選公告</A>";
	  }
	  
      if ( ($system_settings{'allow_print_graduate_pdf'} == 1) or ($SUPERUSER == 1) ) {		///  2015/01/20 全面開放 Nidalap :D~
//           and ($student{'grade'} >=3) and ( preg_match("/^[4]/", $student{'id'}) ) ) {  
        if( !$IS_MOBILE ) {
		  echo InsDoc('R', 'graduate');					///  檢視畢業資格審查表
		}else{											///  行動版測試開放此功能
		  //$m_txt['graduate'] = "<A href='My_Concent_Forms.php?session_id=$session_id&m=1' rel='external'>檢視畢業資格審查表</A>";
		  $m_txt['graduate'] = "<A href='Print_Graduate_pdf.cgi?session_id=$session_id&m=1' rel='external'>"
		    . "<IMG src='../../Graph/mobile/print_graduate.png' width='$ICON_WIDTH'><BR>畢業資格<BR>審查表</A>";
		}
      }
      ...
      if( ($concent_form_allowed == 1)or($system_settings{"current_system_timeline"}>=5) ) {
        if( !$IS_MOBILE ) {
		  ...
		  $m_txt['view_warning'] = "<A href='My_Concent_Forms.php?session_id=$session_id&m=1' rel='external'>我的加簽單</A>";
		}
      }
	  ...
	  if( !$IS_MOBILE ) {
		...
		//$m_txt['change_pwd'] = "<A href='Change_Password00.php?session_id=$session_id&m=1' rel='external'>更改密碼</A>";
	  }
...
//    echo "login time = " . $session_data{login_time} . "<BR>\n";
  ...
  ?>
  <BR>
  <SPAN style="background-color:LIGHTGREEN;padding:2px;margin-bottom:2px; box-shadow: 3px 3px 2px #888888;">
    <A target=_top href="logout.php?session_id=<?PHP echo $session_id; if( $IS_ENGLISH ) echo "&e=1"; ?>">
    <!--  <IMG border=0 src="<?PHP echo $GRAPH_URL ?>logout.jpg" alt="我要登出系統"></A>  -->
      <?PHP echo $txt{"logout_btn"}; ?>
	</A>
  </SPAN>
  </FONT>
  <?PHP 
...
<?PHP
  ...
  {
    global $IS_MOBILE, $m_txt, $session_id, $student, $ICON_WIDTH;
/*	
	if( ($system_settings{'sysstate'} >= 1 ) or ($SUPERUSER == 1)) { }
...
*/
	$m_txt['daily_course'] = "<A href='Daily_Course.php?session_id=$session_id&m=1' rel='external'>";
	$m_txt['daily_course'] .= "<IMG src='../../Graph/mobile/daily_course.png' width='$ICON_WIDTH'><BR>今日課表</A>";
	$m_txt['change_pwd'] = "<A href='Change_Password00.php?session_id=$session_id&m=1' rel='external'>"
						 . "<IMG src='../../Graph/mobile/change_password.png' width='$ICON_WIDTH'><BR>更改密碼</A>";
	
	list($time_string, $time_string2) = gettime($session_data{'login_time'});	
	$welcome_msg  =  $student['name'] . "同學，歡迎！<BR>\n";
	$welcome_msg .= "您本次的登入時間是:[$time_string2]";

	echo Create_jQuery_Mobile_Title_Tag();
	echo "
	  <DIV data-role='content'>
	  <TABLE border=0 width=90%>
	    <TR>
		  <TD align='center' width=33%>" . $m_txt['daily_course'] . "</TD>
		  <TD align='center' width=33%>" . $m_txt['view'] . "</TD>
		  <TD align='center'><A href='announce.php?session_id=$session_id&m=1' rel='external'>
		      <IMG src='../../Graph/mobile/announce.png' width='$ICON_WIDTH'><BR>系統公告</A></TD>
		</TR>
		
		<TR>
		  <TD align='center'>" . $m_txt['add'] . "</TD>
		  <TD align='center'>" . $m_txt['del'] . "</TD>
		  <TD align='center'><A href='system_state.php?session_id=$session_id&m=1' rel='external'>
		      <IMG src='../../Graph/mobile/system_state.png' width='$ICON_WIDTH'><BR>系統狀態</A></TD>
		</TR>
	    <TR>
		  <TD align='center'>" . $m_txt['change_pwd'] . "</TD>
		  <TD align='center'>" . $m_txt['graduate'] . "</TD>
		  <TD align='center'><A href='logout.php?session_id=$session_id&m=1' rel='external'>		  
		    <IMG src='../../Graph/mobile/logout.png' width='$ICON_WIDTH'><BR>登出</A></TD></TR>
		</TR>
	  </TABLE>
	  <DIV>
	";
	echo Create_jQuery_Mobile_Footer_Tag();
	
	die();
  
  }
  //////////////////////////////////////////////////////////////////////////////////////////////
  /////  檢查目前系統與使用者輸入學號是否同為一般生或者專班，
  /////  若否，則轉址到該去的系統。
  /////  2015/06/15  作到一半發現這樣不能完美解決問題，只能影響下方兩個 frame，左上的系統狀態還是沒變。所以暫停！
  function Check_System_Type_And_Redirect($id)
  {
	global $IS_ENGLISH, $IS_MOBILE, $IS_GRA, $IS_SUMMER, $KIKI_URL, $ACCOUNT_NAME, $TEST_STU_ID, $ACCOUNTS;
	
//	if( $ACCOUNT_NAME == $TEST_ACCOUNT )		return;					///  測試系統，不做任何轉址
//	if( $id == $TEST_STU_ID )					return;					///  使用測試帳號，不做任何轉址
	
	//$IS_GRA = 1;
	$IS_SUMMER	= 1;
	
	if( !$IS_GRA and (substr($id,0,1)==5) ) {							///  專班學號誤入一般系統
	  if( $IS_SUMMER )	$dest_url = $KIKI_URL . "~" . $ACCOUNTS[3] . "/cgi-bin/class_new/bookmark.php";
	  else				$dest_url = $KIKI_URL . "~" . $ACCOUNTS[1] . "/cgi-bin/class_new/bookmark.php";
	  
	}else if( $IS_GRA and in_array(substr($id,0,1), array(4,6,8)) ) {	///  一般學號誤入專班系統
	  if( $IS_SUMMER )	$dest_url = $KIKI_URL . "~" . $ACCOUNTS[2] . "/cgi-bin/class_new/bookmark.php";
	  else				$dest_url = $KIKI_URL . "~" . $ACCOUNTS[0] . "/cgi-bin/class_new/bookmark.php";
	}else{
	  return;	
	}
	/////  若執行到此處仍未 return，代表需要轉址到適當的一般生/專班/一般暑修/專班暑修系統。
	$input_html = "";
	foreach( $_POST as $k => $v ) {
	  $input_html .= "<INPUT type='hidden' name=$k value='$v'>\n";	  
	}
	echo "
	  <HTML>
	  <HEAD>
	  ...
	  <BODY>
	    <FORM action='$dest_url' METHOD='POST' id='form1'>
		  $input_html
		  <INPUT type='SUBMIT'>
		</FORM>
	  </BODY>
	...
	$txtall = array(
	  ...
	  'pwd_default'	=> array('c'=>'<FONT color=RED size=-1>您使用預設密碼或是尚未填寫 email 信箱，請先 '
						...
						'e'=>'You are now either still using the default password or havn\'t'
							. ' provided your E-mail address, please ' 
							. '<A target=basefrm href="Change_Password00.php?session_id=' . $session_id . '">Change your password</A><FONT>'), 
