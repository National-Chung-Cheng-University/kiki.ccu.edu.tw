...
#####    2015/05/04  為英文版使用 Read_All_Course_Dept() 抓取開課系所，影響了執行效率（將來有可改善處） Nidalap :D~
#####    2016/02/25  Form_Course_Change_Conflict_Data_Table() 以 foreach 改寫，避開 75 分鐘節次造成誤判之 BUG  Nidalap :D~
...
<P align='right'>
<FONT color=GREEN>" . $txt{'now'} ." $time{time_string}</FONT>
</P>
...
sub Form_System_Choose_Data_Table()
{
  ...
  if(-e $FILENAME){
    ...
    @WARN=<FILE>;
    close(FILE);
    $Table_Data .= "<UL><TABLE border=0 width=70%><TR><TD>" . $txt{'change6'} . "<br>";
    ...
    $Table_Data .= " <th bgcolor=yellow>" . $txt{'cgroup'} . "</th><th bgcolor=yellow>" . $txt{'result'} . "</th>";
	$Table_Data .= " <th bgcolor=yellow>" . $txt{'choose_date'} . "</th></tr>";

    foreach $warn(reverse @WARN){
      $warn=~s/\n//;
      ($dept,$the_id,$group,$state,$choose_date,$choose_time)=split(/\s+/,$warn);
	  
	  $choose_date = "6/17/2016" if($choose_date eq "");
	  ...
	  $bgcolor = $BGCOLORS[$i++%2]  if( $choose_date_last ne $choose_date );
	  ...
	  $Table_Data .= "<th>".$choose_date."</th>";
      $Table_Data .= "</tr>";
	  $choose_date_last = $choose_date;
    }
    $Table_Data .= "</table></UL>";
	
  }else{
    $Table_Data = $txt{'no_affect'} . "<P>\n";
  }
  return($Table_Data);
}
#########################################################################
...
sub Form_Course_Change_Conflict_Data_Table()
{
  ...
  for($i=0;$i< $count-1;$i++)  {
    for($j=$i+1;$j< $count;$j++)  {
	  ...
	  #print("[i, j, cid1, cid2] = [$i, $j, $Courses[$i]{id}, $Courses[$j]{id}]<BR>\n");
	  
	  ###  學生目前所選科目兩兩比對是否衝堂(20160225 改以 foreach 改寫)      
	  foreach $time1 ( @{$temp1{'time'}} ) {
	    foreach $time2 ( @{$temp2{'time'}} ) {
	      $flag += is_Time_Collision($$time1{week}, $$time1{time}, $$time2{week}, $$time2{time});
		}
	  }
...
sub Form_Prerequisite_Course_Data_Table()
{
  ...
  #%course_dept = Read_All_Course_Dept("history");		###  讀取所有科目的所屬系所
    ...
    $Table_Data4 = "
        ...
        <TABLE border=1 width=70%>
          <tr>
            ...
            <th bgcolor=yellow><font size=4>" . $txt{'pre_fail'} . "</th>
			<th bgcolor=yellow><font size=4>" . $txt{'choose_date'} . "</th>
          </tr>
    ";
...
#	  %pre	= Read_Course($$course_dept{$pre}{'dept'}, $pre, '01', "history", "");
	  chop($line);
	  ($cou_id, $grp, $cou_cname, $cou_ename, $pre_id, $pre_cname, $pre_ename, $pass_grade, $stu_grade, $choose_date) = split(/\t/, $line);	  
	  
	  @temp = split(/ /, $choose_date);
	  $choose_date = $temp[0];
	  
	  #print $line . "<BR>\n";
	  #print $$course_dept{$pre}{'dept'} . " - " . $pre . "<BR>\n";
	  #Print_Hash(%pre);
      ...
      $Table_Data4 .= "<TR bgcolor='$bgcolor'><TD>$cou_name ($cou_id)</TD><TD>$pre_name ($pre_id)</TD><TD>$choose_date</TD></TR>";
	  $choose_date_last = $choose_date;
    }
    $Table_Data4 .= "</TABLE></UL>";
  }else{
...
sub Form_Duplicate_Course_Data_Table()
{
  ...
  if(-e $dup_file) {
    ...
    foreach $line (@lines) {
      $line =~ s/\n//;
      ($c_id, $c_group, $c_name, $choose_date) = split(/\t/, $line);
	  
	  @temp = split(/ /, $choose_date);
	  $choose_date = $temp[0];
	  
	  $bgcolor = $BGCOLORS[$i++%2]  if( $choose_date_last ne $choose_date );
      $Table_Data5 .= "<TR bgcolor='$bgcolor'><TD>$c_name($c_id, $c_group)</TD><TD>$choose_date</TD></TR>";
	  
	  $choose_date_last = $choose_date;
	}

    $Table_Data5 .= "</TABLE></UL>";
  }else{
...
sub Init_Text_Values
{
  ...
  %txtall = (
    'title'		=> {'c'=>'人數篩選篩選公告（空白表示無相關資料）', 
					'e'=>'Course Screening Result(There is no data if left blank)'},
	'now'		=> {'c'=>'目前時間是:', 'e'=>'Current Time:'},
	'change'	=> {'c'=>'科目異動公告（空白表示無相關資料）', 
					'e'=>'Course update announcement(There is no data if left blank)'},
	'change2'	=> {'c'=>'以下科目因科目異動影響導致衝堂（空白表示無相關資料）', 
					'e'=>'Course update resulting in schedule conflict(There is no data if left blank)'},
	'change3'	=> {'c'=>'以下科目因不符先修條件導致退選（空白表示無相關資料）', 
					'e'=>'Course dropped by the system: prerequisite requirement(There is no data if left blank)'},
	'change4'	=> {'c'=>'以下科目因重複修習導致退選（空白表示無相關資料）', 
					'e'=>'Course dropped by the system: courses already taken(There is no data if left blank)'},
	'change5'	=> {'c'=>'特殊情形通知（空白表示無相關資料）', 'e'=>'Other notice(There is no data if left blank)'},
	...
	'choose_date'=> {'c'=>'篩選日期', 'e'=>'Screening Date'},
	...
	'pre_fail'	=> {'c'=>'不合先修條件科目(未修習或不及格)', 'e'=>'Prerequisite courses that you have not selected nor passed'},
