...
#################################################################################
#####  新生通識英語課程批次載入
#####  讀取(大一新生)英檢分級資料檔案，以學系為單位，排除其必修課程時段，
#####  批次分派大一新生英文課程，批次加選。
#####  已經解決的問題：
#####    * 支援系所的必修課     2012/05/17(?)
#####  尚未解決的問題：
...
#####  Updates:
#####    2011/05/17 Created by Nidalap :D~
#####    2012/07/25 讀取各系「中文課程」時段，排除此時段。  Nidalap :D~
#####    2012/08/30 排除英檢成績檔中非大一的學生。 <--- 沒必要，本來就會過濾。 Nidalap :D~
#####    2014/09/10 加回限制必須大一的條件。此次改採驗證學號前三碼的方式檢核。  Nidalap :D~
...
if( $Input{"debug"} == 1 ) {
  ...
  print("<A href='$script_name?debug=0&password=$pass'>隱藏所有偵錯訊息</A>\n");
}else{
  @show_debug_msg = (0, 0, 0, 0, 0);
  print("<A href='$script_name?debug=1&password=$pass'>顯示所有偵錯訊息</A>\n");
}
...
sub Find_All_Dept_Occupied_Time
{
  ...
  foreach $dept (@dept) {
    ...
    ###  讀取所有學系所有課程，將必修與通識中文課程時段抓出來
    foreach $cour (@course) {
      %cou = Read_Course($dept, $$cour{id}, $$cour{group}, "", "", "");
      ###  只考慮必修課程，以及通識中文
      next if( ($cou{property} != 1) and ($$cour{id} !~ /^7101/) );
...
{
  my $file = $DATA_PATH . "Grade/a14teng_gen_class.txt";
  my @lines, $id, $cname, $level, %eng_level_namelist, $tmp_count;
  my $sid_like = "4" . substr($YEAR, 1,2);
  
#  $sid_like -= 1;
#  print "sid_like : $sid_like<BR>\n";
...
#  exit();
  ...
  foreach $line (@lines) {
    ...
    $stu_level{$id} = $level;
    if( $$S{$id}{grade} != 1) { 
#	if( substr($id,0,3) != $sid_like ) {
#      print "$id : " . $$S{$id}{grade} . "<BR>\n";
#      print "$id : $sid_like<BR>\n";
#	  exit();
	  
	  
      next;					###  排除非大一的學生  2012/08/30 Nidalap :D~
    }
    if( $level == 1 ) {
