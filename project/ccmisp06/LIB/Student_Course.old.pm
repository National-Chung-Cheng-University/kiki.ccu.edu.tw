...
#        for($i=0; defined($temp[$i]); $i+=5){
#            $Course_of_the_Student[$index]{id}=$temp[$i];
#            $Course_of_the_Student[$index]{dept}=$temp[$i+1];
#            $Course_of_the_Student[$index]{group}=$temp[$i+2];
#            $Course_of_the_Student[$index]{credit}=$temp[$i+3];
#            $Course_of_the_Student[$index]{property}=$temp[$i+4];
#        }
...
#    print("[year, term]=[$year, $term], <BR>Locating $STUDENT_OF_COURSE_PATH ......<BR>\n");
...
sub Delete_Student_Course
{
    ...
    ####    寫回記錄檔    ####
    open(FILE,">$STUDENT_OF_COURSE_PATH/$Course_File") or
         die("Cannot open file $Course_File!!\n");
...
#####  用到global: %Student
#####  Updates: 2011/07/30 從 Add_Course01.cgi 搬過來，「科目是否需要加簽」檢核也會用到 Nidalap :D~
############################################################################################
sub Check_Ban_Limit
{
