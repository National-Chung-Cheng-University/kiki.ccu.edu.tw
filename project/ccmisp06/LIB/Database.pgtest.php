...
//    $password="HowDidTheyFindThis!?";
    $password = "passwordChanging@academic";
    
    $connection_string = "host=$host port=5432 user=$user password=$password ";
    
    if( $db_kiki == 1 ) {
      $connection_string .= "dbname=$KIKI_DB_NAME";
    }else{
...
//    if ($link=sybase_pconnect($host,$user,$password)) {		/// 資料庫連結成功
    if( $link = pg_pconnect($connection_string) ) {
...
//    $select_db_result = sybase_select_db($database);
    $select_db_result = pg_select_db($database);
*/
    $debug = 0;
    if($debug) {
